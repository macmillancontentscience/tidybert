...
The tidybert package, along with {[torchtransformers](https://github.com/macmillancontentscience/torchtransformers)}, provides a {[torch](https://torch.mlverse.org/)}-based R interface to the BERT family of language models. 
It is the successor to the TensorFlow-based package {[RBERT](https://github.com/jonathanbratt/RBERT)}.
...
To keep it general, suppose we have sequences with multiple segments, with each segment in a different column in the table.
...
Let's first tokenize the input text. 
...
tokenized <- torchtransformers::tokenize_bert(text_inputs$segment1,
                                              ...
                                              simplify = TRUE,
                                              n_tokens = n_tokens)
# For now, we will also need the non-simplified tokenized text, for the token
# names. A forthcoming {torchtransormers} update will make this unnecessary. 
tokenized_names <- torchtransformers::tokenize_bert(text_inputs$segment1,
                                                    text_inputs$segment2,
                                                    simplify = FALSE,
                                                    n_tokens = n_tokens)
```
...
```{r}
bert_model <- torchtransformers::make_and_load_bert(model_name = "bert_base_uncased")
...
The tidybert package provides functions to further process the model output into a tidy format:

```{r}
tidy_out <- tidybert::tidy_bert_output(bert_output, tokenized_names)
```
...
## RBERTviz compatibility

The tidied outputs are compatible with the visualization functions from the RBERTviz package.

```{r, fig.width=6, fig.height=4}
# compatible with RBERTviz
tidy_out$embeddings %>% 
  dplyr::filter(token %in% c("it", "chicken", "road"),
                layer_index %in% c(12)) %>% 
  dplyr::mutate(it_was = dplyr::case_when(sequence_index == 1 ~"wide", 
                                          sequence_index == 2 ~"tired",
                                          sequence_index == 3 ~"raining")
                ) %>% 
  RBERTviz::display_pca(color_field = "it_was", disambiguate_tokens = FALSE)
```

```{r}
# The attention viewer also works
RBERTviz::visualize_attention(tidy_out$attention, sequence_index = 1)
```
...
```{r, eval = FALSE}
# Here's where we'll get our training data from:
synant_url <- "https://www.gutenberg.org/dirs/5/1/1/5/51155/51155-csv.csv"
synant <- readr::read_csv(synant_url)
...
# preserved.
# Let's also pivot so that the synonyms and antonyms are all in the same column.
synant_cleaner <- synant_clean %>% 
...
train_test_split <- rsample::initial_split(synant_cleaner, prop = 0.9)
...
mean(preds$.pred_class == as.factor(testing_set$target))
# [1] 0.7970974
